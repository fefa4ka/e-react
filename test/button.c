...
test(long_push)
{
    ...
    loop(clk, switcher)
    {
        if (enabled) {
            ...
            apply(IO, led, _({.level = Button_isPressed(&pusher)}));
        }
    }
}
...
void long_push()
{
    ...
    test_assert(Button_isPressed(&switcher) == false,
                "Switcher unpushed at first");
    test_assert(Button_isPressed(&pusher) == false, "Pusher unpushed at first");
    test_assert(hw.io.get(&switcher_pin) == hw.io.get(&pusher_pin),
                "Pins should be equal %d = %d", hw.io.get(&switcher_pin),
                hw.io.get(&pusher_pin));
    test_assert(hw.io.get(&pusher_pin) == 0,
    ...
    usleep(500000);
    test_assert(hw.io.get(&switcher_pin) == 1,
    ...
    test_assert(hw.io.get(&switcher_pin) == 0,
    ...
    test_assert(hw.io.get(&pusher_pin) == 0,
                "Pusher pin sholud be 0, not %d before swither off",
                hw.io.get(&pusher_pin));
    test_assert(Button_isPressed(&pusher) == false,
