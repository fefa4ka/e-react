...
bool
Scheduler_enqueue (Component *instance, unsigned int    timeout_us,
                   void (*callback) (void *args), void *args)
{
    ...
    event new_event = { timeout_us, now, callback, args };
...
void
event_prioritify (struct events_queue *queue, unsigned int now,
                  event *new_event)
{
    // Use priority queue
    ...
    event swap = { 0 };
    while (index > 0
           && event_compare (now, &events[index], &events[PARENT (index)])
                  > 0) {
        swap                   = events[index];
...
void
event_heapify (struct events_queue *scheduler, unsigned int now,
               unsigned char idx)
{
    event  swap   = { 0 };
...
willUpdate (Scheduler)
{
    ...
    if (state->next_event.callback) {
        props->timer->off ();
        event_prioritify (props->queue, props->timer->get (),
                          &state->next_event);
    }
