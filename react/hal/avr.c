...
int16_t adc_readConvertion(void *channel);
...
uint16_t timer_get();
void     timer_set(uint16_t ticks, void (*callback)(void *args), void *args);
void     timer_off();
uint16_t timer_usFromTicks(uint16_t ticks);
 ...
 * pin toggles.
 */
void io_isr_mount(void *pin, struct callback *callback)
{
    //    pin_t *Pin = (pin_t *)pin;
...
inline void adc_selectChannel(void *channel)
{
...
inline void adc_startConvertion(void *channel)
{
...
inline bool adc_isConvertionReady(void *channel)
{
    return (ADCSRA & (1 << ADSC)) == 0;
}

inline int adc_readConvertion(void *channel) { return (ADC); }
...
inline bool uart_isDataReceived() { return (UCSR0A & (1 << RXC0)); }

inline bool uart_isTransmitReady() { return (UCSR0A & (1 << UDRE0)); }

inline void uart_transmit(unsigned char data) { UDR0 = data; }

inline unsigned char uart_receive() { return UDR0; }
...
inline bool spi_isDataReceived()
{
...
inline bool spi_isTransmitReady() { return !(SPI_SPSR & (1 << SPI_SPIF)); }

inline void spi_transmit(unsigned char data) { SPI_SPDR = data; }

inline unsigned char spi_receive() { return SPI_SPDR; }
...
inline uint16_t timer_get() { return TCNT1; }

void (*_timer_func)(void *args);
void *_timer_func_args = 0;

ISR(TIMER1_COMPA_vect) { _timer_func(_timer_func_args); }
...
uint16_t timer_usFromTicks(uint16_t ticks) { return ticks >> 1; }
