...
#define React_LifeCycle_Prepare(Type, instance, stage) hw_isr_disable()
#define React_LifeCycle_Finish(Type, instance, stage)  hw_isr_enable()
...
#define React_LifeCycle(Type, stage)                                           \
    ...
    {                                                                          \
        ...
        React_LifeCycle_Prepare(Type, instance, stage);                        \
        Type##_inline_##stage(&self->instance, &self->props, &self->state);    \
        React_LifeCycle_Finish(Type, instance, stage);                         \
    }                                                                          \
    React_LifeCycle_Header(Type, stage)
...
#define willMount(Type)                                                        \
    ...
    {                                                                          \
        ...
        React_LifeCycle_Prepare(Type, instance, willMount);                    \
        Type##_inline_willMount(&self->instance, &self->props, &self->state,   \
                                next_props);                                   \
        React_LifeCycle_Finish(Type, instance, willMount);                     \
    }                                                                          \
    React_UpdateCycle_Header(Type, willMount, void)
...
#define willUpdate(Type)                                                       \
    ...
    {                                                                          \
        ...
        if (!next_props_ptr)                                                   \
            next_props_ptr = &self->props;                                     \
        React_SelfNext(Type, instance);                                        \
        React_LifeCycle_Prepare(Type, instance, willUpdate);                   \
        Type##_inline_willUpdate(&self->instance, &self->props, &self->state,  \
                                 next_props);                                  \
        React_LifeCycle_Finish(Type, instance, willUpdate);                    \
        if (&self->props != next_props)                                        \
            self->props = *next_props;                                         \
    }                                                                          \
    React_UpdateCycle_Header(Type, willUpdate, void)
...
#define React_Define(Type, name) React_Define_WithProps(Type, name, _({0}))
...
#define apply(Type, name, propsValue)                                          \
    {                                                                          \
        if (name.instance.stage == REACT_STAGE_RELEASED                        \
            ...
            React_LifeCycle_Prepare(Type, &name, nextProps);          \
            Type##_props_t next_props = propsValue;                            \
            React_LifeCycle_Finish(Type, &name, nextProps);           \
